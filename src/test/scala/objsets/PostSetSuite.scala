...
class PostSetSuite extends munit.FunSuite {
  ...
  //variant test
  val set6_0: PostSet = new Empty()
  val set6_1: PostSet = set6_0.incl(new Post("test user", "test", 19))
  val set6_2: PostSet = set6_1.incl(new Post("test user", "test", 19))
  ...
  test("incl: avoid duplicates in set6") {
    assertEquals(size(set6_2.filter(post => post.text == "test" && post.user == "test user")), 1)
  }
